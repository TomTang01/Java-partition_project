...
public class PartitionOracle {
    ...
    public static CounterExample findCounterExample(Partitioner p) {
        ...
        for(int i=4; i<94; i++){
            int arrLength = i / 3;
            for(int lowerBound=0; lowerBound<arrLength; lowerBound++){
                for(int upperBound=lowerBound+1; upperBound<=arrLength; upperBound++){
                    testArray = generateInput(arrLength);
                    afterArray = Arrays.copyOf(testArray, arrLength);
                    int pivot = p.partition(afterArray, lowerBound, upperBound);
                    String reason = isValidPartitionResult(testArray, lowerBound, upperBound, pivot, afterArray);
                    if(reason != null){
                        return new CounterExample(testArray, lowerBound, upperBound, pivot, afterArray, reason);
                    }
                }
            }
        }
